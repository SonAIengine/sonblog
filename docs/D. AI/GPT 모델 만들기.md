...
logits[:,-1,:] 연산을 수행할 때,
- : (첫 번째 차원): 모든 배치를 선택한다.(여기선 1개).
- -1 (두 번째 차원): 각 배치의 마지막 시퀀스/토큰을 선택한다.
- : (세 번째 차원): 선택된 토큰의 모든 로짓 값을 가져온다.

이 연산으로 인해 두 번째 차원(시퀀스 차원)이 제거된다. 왜냐하면 -1 로 인해 이 차원에서 단 하나의 요소만 선택되기 때문이다.

결과적으로,
- 첫 번째 차원은 그대로 유지된다. (여전히 1개의 배치).
- 두 번째 차원은 사라진다. (단일 시퀀스/토큰 선택으로 인해).
- 세 번째 차원은 그대로 유지된다. (4개의 로짓 값).

따라서 결과 텐서 형태는 (1,4)가 되며, 이는 2차원 텐서이다.

좋습니다. `logits[:, -1, :]`의 역할을 더 쉽게 이해할 수 있도록 하나씩 풀어서 설명해볼게요.
...
이 의미는 **"각 배치의 마지막 토큰에 대한 예측 로짓만 뽑겠다"는 뜻이에요.
...
즉, **가장 마지막 토큰 위치에서, 다음 토큰이 무엇일지에 대한 예측 확률을 보고 싶어서 이 부분을 뽑는 것**이에요.

---

### 📈 왜 마지막 토큰만 쓰는 걸까?

`generate()` 함수는 **현재까지 입력된 문장을 기반으로 다음 글자 하나를 생성**하는 과정이죠.
...
그러면 모델은 `안`, `녕` 각각의 위치에 대해 다음 글자를 예측하지만, 우리는 마지막 글자인 `"녕"` 뒤에 어떤 글자가 나올지를 알고 싶어요.
...
이걸 softmax 후 multinomial sampling을 통해 **다음 글자 하나**를 샘플링합니다.
...
|코드|의미|
|---|---|
|`logits.shape = (1, 3, 4)`|1개 문장에서 3글자를 예측했고, 어휘 수는 4개|
|`logits[:, -1, :]`|마지막(3번째) 글자에 대한 다음 글자 예측 점수만 추출|
|결과 shape: `(1, 4)`|어휘 4개에 대한 확률 벡터|

---

### 🧠 한 줄 요약

> `logits[:, -1, :]`는 **지금까지 생성된 문장 중 마지막 글자 위치에서, 다음에 어떤 글자가 나올지 예측한 확률 값(logits)을 뽑기 위한 연산**입니다.
