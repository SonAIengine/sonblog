...
- [llama.cpp 서버 운영기: ROCm GPU에서의 삽질과 해결](XGEN/llama-cpp-server-ops-story-rocm-gpu-troubleshoot-fix.md)
- [Qdrant 하이브리드 검색: Sparse + Dense 벡터 통합](XGEN/qdrant-hybrid-search-sparse-dense-vector-integration.md)
- [Sparse Vector와 Full-Text Index 하이브리드 검색 구현](XGEN/sparse-vector-full-text-index-hybrid-search-impl.md)
- [vLLM 모델 배포: 샘플링 파라미터 튜닝 가이드](XGEN/vllm-model-deploy-sampling-parameter-tuning-guide.md)
- [GPU 상태 모니터링 및 자동 모델 배포 시스템](XGEN/gpu-status-monitoring-auto-model-deploy-system.md)
...
- [GliNER과 DPO-LoRA를 활용한 모델 파인튜닝](fine-tuning/gliner-dpo-lora-finetuning.md)
- [OJT 리팩토링과 Kotaemon RAG 구현기](fine-tuning/ojt-refactoring-kotaemon-rag-impl.md)
