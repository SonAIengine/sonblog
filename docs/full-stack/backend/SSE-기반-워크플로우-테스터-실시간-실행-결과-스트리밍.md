...
- [SSE 스트리밍으로 대규모 배치 워크플로우 결과 전달하기](../../ai/XGEN/SSE-스트리밍으로-대규모-배치-워크플로우-결과-전달하기.md): batch_results에서 progress-only 방식으로의 전환, 취소 구현, Redis 세션 관리
- [Redis 기반 SSE 세션 상태 공유: 멀티 POD 환경](Redis-기반-SSE-세션-상태-공유-멀티-POD-환경.md): K8s 멀티 POD에서 SSE 세션을 Redis로 동기화하는 방법
