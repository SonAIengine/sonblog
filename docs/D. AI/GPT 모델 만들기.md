...
`__init__` 메서드는 클래스 인스턴스가 생성될 때(예를 들어, model = semiGPT(ko_vocab_size) 선언할 때) 자동으로 호출되는 특별한 메서드이다. 이를 파이썬 생성자(constructor)라고 부르는데, 쉽게 말해 이 메서드는 새로운 물건을 만들 때 기본 설정하는 것과 같다.
