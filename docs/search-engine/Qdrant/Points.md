---
title: "Points"
description: "Qdrant에서 가장 핵심적인 데이터 단위는 포인트(Point)이다. 포인트는 벡터와 페이로드(payload)를 함께 포함하는 하나의 레코드이며, Qdrant에서의 유사도 검색, 필터링 기반 검색, 데이터 업데이트, 삭제 및 조회 등의 모든 연산은 이 포인트를 중심으로 이루어진다."
date: 2025-07-15
tags:
  - 검색엔진
  - 벡터 검색
