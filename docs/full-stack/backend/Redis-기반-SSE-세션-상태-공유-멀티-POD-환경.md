...
- [SSE 기반 워크플로우 테스터: 실시간 실행 결과 스트리밍](SSE-기반-워크플로우-테스터-실시간-실행-결과-스트리밍.md): 배치 버퍼링과 DB 분리 전략으로 SSE 트래픽을 최소화한 구현기
- [SSE 스트리밍으로 대규모 배치 워크플로우 결과 전달하기](../../ai/XGEN/SSE-스트리밍으로-대규모-배치-워크플로우-결과-전달하기.md): 100개 이상 테스트 케이스의 배치 처리를 SSE로 실시간 전달하는 아키텍처
