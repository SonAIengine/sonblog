...
지금까지는 CPU로 학습했는데, 이제 GPU를 이용해 학습해 보겠다. GPU를 이용해 학습하려면 데이터와 모델을 반드시 GPU로 전송해야 한다. 이를 위해 파이토치에서는 `to` 메서드를 제공한다. `to(device)` 명령어를 사용하면 데이터와 모델을 간편하게 GPU로 전송해 GPU 연산을 수행할 수 있다. 먼저 다음은 device